...
from openaq_api.v3.routers.sensors import SensorPathQuery
...
async def fetch_sensor_measurements(q, db):
    query = QueryBuilder(q)
    dur = "01:00:00"
    expected_hours = 1
...
class SesnorMeasurementsQueries(
    Paging,
    SensorPathQuery,
    DateFromQuery,
    DateToQuery,
...
@router.get(
    ...
    response_model=MeasurementsResponse,
    summary="Get measurements by sensor",
    description="Provides a list of measurements by sensors ID",
)
async def sensor_measurements_get(
    measurements: Annotated[
        SesnorMeasurementsQueries, Depends(SesnorMeasurementsQueries.depends())
    ],
    db: DB = Depends(),
):
    response = await fetch_sensor_measurements(measurements, db)
    return response
