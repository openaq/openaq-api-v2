...
class LocationSensorQuery(QueryBaseModel):
    ...
    def where(self):
        return "m.locations_id = :locations_id"
...
@router.get(
    ...
    summary="Get a sensors by location ID",
...
async def sensors_get(
    ...
    response = await fetch_location_sensors(location_sensors, db)
    return response

async def fetch_location_sensors(q, db):
...
location AS (
    SELECT 
        jsonb_array_elements(sensors::jsonb) ->> 'id' AS sensor_id
    FROM 
...
sensor AS (
    ...
    FROM hourly_data m
    WHERE m.sensors_id IN (SELECT sensor_id::int FROM location)
    GROUP BY 1
)
SELECT 
    c.sensors_id as id,
...
WHERE s.sensors_id IN (SELECT sensor_id::int FROM location);
