...
from openaq_fastapi.models.logging import (HTTPLog, LogType,
                                           TooManyRequestsLog, UnauthorizedLog)
...
class RateLimiterMiddleWare(BaseHTTPMiddleware):
    ...
    async def request_is_limited(self, key: str, limit: int):
        print(key, limit)
        if await self.redis_client.setnx(key, limit):
    ...
    async def dispatch(
        ...
        if self.limited_path(route) and await self.request_is_limited(key, limit):
            print("LIMIT")
            logging.info(
                TooManyRequestsLog(
                    request=request,
