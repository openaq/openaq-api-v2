...
class ProjectsPath(Project, Measurands, APIBase, Country):
    project_id: int = Path(
...
class Projects(Project, Measurands, APIBase, Country):
    ...
    order_by: Optional[str] = "lastUpdated"
...
async def projects_by_id(
    project_id: int = Path(
        ...,
        description="Limit results by a certain project using two digit project ID. e.g. 13",
        example=13,
    ),
    projects: ProjectsPath = Depends(),
    db: DB = Depends(),
):
    q = f"""
        WITH bysensor AS (
