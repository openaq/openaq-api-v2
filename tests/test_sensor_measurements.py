...
class TestMeasurements:
    ...
    def test_date_filter_good(self, client):
        ...
        response = client.get(
            f"/v3/sensors/7/measurements?datetime_from=2023-03-05&datetime_to=2023-03-06"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        row = data[0]
    ...
    def test_date_filter_aggregated_hourly_good(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/measurements/hourly?datetime_from=2023-03-05&datetime_to=2023-03-06"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        assert len(data) == 24
...
class TestHours:
    ...
    def test_date_filter_good(self, client):
        ## 7 is the only hourly sensor
        response = client.get(
            f"/v3/sensors/7/hours?datetime_from=2023-03-05T00:00:00&datetime_to=2023-03-06T00:00:00"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        row = data[0]
    ...
    def test_aggregated_daily_reversed_dates_bad(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/hours/daily?datetime_from=2023-03-06&datetime_to=2023-03-05"
        )
        assert response.status_code == 422

    def test_aggregated_daily_dates_good(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/hours/daily?datetime_from=2023-03-05&datetime_to=2023-03-06"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        assert len(data) > 0
    ...
    def test_aggregated_yearly_good_with_dates(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/hours/yearly?datetime_from=2022-01-01&datetime_to=2023-01-01"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        assert len(data) == 1
    ...
    def test_aggregated_hod_dates_good(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/hours/hourofday?datetime_from=2023-03-01&datetime_to=2023-04-01"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        assert len(data) == 24
    ...
    def test_aggregated_hod_timestamps_good(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/hours/hourofday?datetime_from=2023-03-01T00:00:01&datetime_to=2023-04-01T00:00:01"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        assert len(data) == 24
    ...
    def test_aggregated_hod_timestamptzs_good(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/hours/hourofday?datetime_from=2023-03-01T00:00:00Z&datetime_to=2023-04-01T00:00:00Z"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        assert len(data) == 24
    ...
    def test_aggregated_moy_good(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/hours/monthofyear?datetime_from=2022-01-01T00:00:00Z&datetime_to=2023-01-01T00:00:00Z"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        assert len(data) == 12
...
class TestDays:
    ...
    def test_good_with_dates(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/days?date_from=2023-03-05&limit=1"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        assert len(data) == 1
    ...
    def test_aggregated_monthly_good(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/days/monthly?date_from=2022-01-01&date_to=2022-12-31"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        assert len(data) == 12
    ...
    def test_aggregated_yearly_good(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/days/yearly?date_from=2022-01-01&date_to=2022-12-31"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        for d in data:
    ...
    def test_aggregated_dow_good(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/days/dayofweek?date_from=2022-01-01&date_to=2022-12-31"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        assert len(data) == 7
    ...
    def test_aggregated_moy_good(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/days/monthofyear?date_from=2022-01-01&date_to=2022-12-31"
        )
        assert response.status_code == 200
        data = json.loads(response.content).get("results", [])
        assert len(data) == 12
...
class TestYears:
    ...
    def test_good_with_dates(self, client):
        response = client.get(
            f"/v3/sensors/{sensors_id}/years?date_from=2022-01-01&limit=1"
        )
        data = json.loads(response.content).get("results", [])
        assert len(data) == 1
        row = data[0]
